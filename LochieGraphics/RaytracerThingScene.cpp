...
void RaytracerThingScene::Update(float delta)
{
	...
	bool prevReloadKeyDown = reloadKeyDown;
	reloadKeyDown = glfwGetKey(SceneManager::window, GLFW_KEY_R) == GLFW_PRESS;
	if (reloadKeyDown && !prevReloadKeyDown)
	{
		reload = true;
	}
...
void RaytracerThingScene::Draw(float delta)
{
	...
	if (firstDraw || alwaysRedraw)
	{
		...
		auto camera_center = camera->transform.getPosition();
		...
		tracerShader->setInt("camera.width", imageWidth);
		tracerShader->setInt("camera.height", imageHeight);
		tracerShader->setInt("camera.samples", samplePerPixel);
		...
		tracerShader->setVec3("camera.center", camera_center);
...
{
	ImGui::Checkbox("Always redraw", &alwaysRedraw);
	ImGui::SliderFloat("timer", &timer, 0.0f, 4.0f);
	ImGui::SliderInt("samplePerPixel", &samplePerPixel, 1, 1000);
}
